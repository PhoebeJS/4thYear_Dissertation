...
# CI
all_coef <- coef(GAM_soc_regen_score_6_cv)
all_se <- sqrt(diag(vcov(GAM_soc_regen_score_6_cv)))
parametric_terms <- names(summary(GAM_soc_regen_score_6_cv)$p.coeff)
...
# CI
all_coef <- coef(GAM_soc_regen_conv_6_cv)
all_se <- sqrt(diag(vcov(GAM_soc_regen_conv_6_cv)))
parametric_terms <- names(summary(GAM_soc_regen_conv_6_cv)$p.coeff)
...
# CI
all_coef <- coef(GAM_soc_org_conv_6_cv)
all_se <- sqrt(diag(vcov(GAM_soc_org_conv_6_cv)))
parametric_terms <- names(summary(GAM_soc_org_conv_6_cv)$p.coeff)
...
# CI
all_coef <- coef(GAM_microbial_biomass_regen_5_cv_2)
all_se <- sqrt(diag(vcov(GAM_microbial_biomass_regen_5_cv_2)))
parametric_terms <- names(summary(GAM_microbial_biomass_regen_5_cv_2)$p.coeff)
...
# CI
all_coef <- coef(GAM_microbial_biomass_regen_conv_5_cv_2)
all_se <- sqrt(diag(vcov(GAM_microbial_biomass_regen_conv_5_cv_2)))
parametric_terms <- names(summary(GAM_microbial_biomass_regen_conv_5_cv_2)$p.coeff)
...
# CI
all_coef <- coef(GAM_microbial_biomass_org_conv_5_cv_2)
all_se <- sqrt(diag(vcov(GAM_microbial_biomass_org_conv_5_cv_2)))
parametric_terms <- names(summary(GAM_microbial_biomass_org_conv_5_cv_2)$p.coeff)
...
# CI
all_coef <- coef(GAM_bulk_density_regen_conv_3_cv)
all_se <- sqrt(diag(vcov(GAM_bulk_density_regen_conv_3_cv)))
parametric_terms <- names(summary(GAM_bulk_density_regen_conv_3_cv)$p.coeff)
...
# CI
all_coef <- coef(GAM_bulk_density_org_conv_3_cv)
all_se <- sqrt(diag(vcov(GAM_bulk_density_org_conv_3_cv)))
parametric_terms <- names(summary(GAM_bulk_density_org_conv_3_cv)$p.coeff)
...
# CI
all_coef <- coef(GAM_eff2_regen_score_2_cv_2)
all_se <- sqrt(diag(vcov(GAM_eff2_regen_score_2_cv_2)))
parametric_terms <- names(summary(GAM_eff2_regen_score_2_cv_2)$p.coeff)
...
# CI
all_coef <- coef(GAM_eff2_regen_conv_2_cv_2)
all_se <- sqrt(diag(vcov(GAM_eff2_regen_conv_2_cv_2)))
parametric_terms <- names(summary(GAM_eff2_regen_conv_2_cv_2)$p.coeff)
...
# CI
all_coef <- coef(GAM_eff2_org_conv_2_cv_2)
all_se <- sqrt(diag(vcov(GAM_eff2_org_conv_2_cv_2)))
parametric_terms <- names(summary(GAM_eff2_org_conv_2_cv_2)$p.coeff)
...
Table for raw SH variables associated with regen/conv and organic/conv
```{r}
...
# Helper function to format mean ± SE
format_mean_se <- function(mean, se) {
  sprintf("%.2f ± %.2f", mean, se)
}

# Regenerative classification summary
regen_summary <- data.farm %>%
  group_by(Score.Result) %>%
  summarise(
    mean_SOC = mean(SOC_3000_mean, na.rm = TRUE),
    se_SOC = sd(SOC_3000_mean, na.rm = TRUE) / sqrt(n()),
    mean_biomass = mean(microbial_biomass_mean, na.rm = TRUE),
    se_biomass = sd(microbial_biomass_mean, na.rm = TRUE) / sqrt(n()),
    mean_bd = mean(bulk_density_mean, na.rm = TRUE),
    se_bd = sd(bulk_density_mean, na.rm = TRUE) / sqrt(n())
  ) %>%
  mutate(
    SOC = format_mean_se(mean_SOC, se_SOC),
    Biomass = format_mean_se(mean_biomass, se_biomass),
    Bulk_Density = format_mean_se(mean_bd, se_bd)
  ) %>%
  select(Score.Result, SOC, Biomass, Bulk_Density) %>%
  pivot_longer(-Score.Result, names_to = "Metric", values_to = "value") %>%
  pivot_wider(names_from = Score.Result, values_from = value)

# Organic classification summary
org_summary <- data.farm %>%
  group_by(organic_cert) %>%
  summarise(
    mean_SOC = mean(SOC_3000_mean, na.rm = TRUE),
    se_SOC = sd(SOC_3000_mean, na.rm = TRUE) / sqrt(n()),
    mean_biomass = mean(microbial_biomass_mean, na.rm = TRUE),
  ...
  select(organic_cert, SOC, Biomass, Bulk_Density) %>%
  pivot_longer(-organic_cert, names_to = "Metric", values_to = "value") %>%
  pivot_wider(names_from = organic_cert, values_from = value)

# Combine and rename
final_table <- regen_summary %>%
  full_join(org_summary, by = "Metric") %>%
  select(Metric, Regenerative, Conventional, ORG, CONV) %>%
  rename(
    `Organic` = ORG,
    `Conventional (Organic)` = CONV,
    `Conventional (Regen)` = Conventional
  )%>%
  mutate(Metric = case_when(
    Metric == "SOC" ~ "Soil Organic Carbon (Mg ha⁻¹)",
    Metric == "Biomass" ~ "Microbial Biomass (ng g⁻¹)",
    Metric == "Bulk_Density" ~ "Bulk Density (g cm⁻³)",
    TRUE ~ Metric
  ))
...
ft <- flextable(final_table) %>%
  ...
  add_header_row(
    values = c("", "Regenerative Classification", "Organic Classification"),
    colwidths = c(1, 2, 2)
  ) %>%
  font(fontname = "Times", part = "all") %>%                   # serif font
  fontsize(size = 10, part = "all") %>%                        # base size
  color(color = "black", part = "all") %>%                     # text color
  bg(bg = "white", part = "all") %>%                           # background color
  border_remove() %>%                                          # remove default borders
  border_outer(border = fp_border(color = "black", width = 1)) %>%  # outer border
  border_inner(border = fp_border(color = "black", width = 0.5)) %>%# inner grid
  bold(part = "header") %>%                                    # bold column headers
  bold(i = NULL, j = 1, part = "body", bold = TRUE) %>%        # bold first column (metrics)
  padding(padding = 4, part = "all") %>%                       
  autofit()

ft
ft <- ft %>%
  align(j = 2:5, align = "center", part = "all")  # columns 2 to 5 (adjust if needed)
ft

#install.packages("webshot2")
library(webshot2)

# Save flextable as HTML then image
save_as_image(ft, path = "results_table_alt_style.png", zoom = 2)
